...
func (s *RoomService) selectReactionLocked(active *ActiveRoom, fromSeat int, tile model.Tile, source string) []PendingClaim {
	order := orderedSeatsFrom(fromSeat)
	huClaims := []PendingClaim{}
	pengGangClaims := []PendingClaim{}
	chiClaim := PendingClaim{Seat: -1}

	for _, seat := range order {
		if s.canHuOnClaimLocked(active.game, seat, tile, source) {
			huClaims = append(huClaims, PendingClaim{
				Seat: seat,
		...
		if source == "discard" && s.canPengLocked(active.game, seat, tile) {
			options = append(options, model.GameActionOption{Type: model.ActionPeng})
		}
		if len(options) > 0 {
			pengGangClaims = append(pengGangClaims, PendingClaim{
				Seat:    seat,
	...
	if len(pengGangClaims) > 0 {
		return pengGangClaims
	}
	if chiClaim.Seat >= 0 {
		return []PendingClaim{chiClaim}
	}
	return nil
}
...
func (s *RoomService) canHuOnClaimLocked(game *GameState, seat int, tile model.Tile, source string) bool {
	if tile.Code == "P" {
		return false
	}
	for _, card := range game.Hands[seat] {
		if card.Code == "P" && !isEarthHuLocked(game, seat, source) {
			return false
		}
	}
